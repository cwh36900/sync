...
public class WriterExcelUtil {
    ...
    public static boolean writerExcel(String path, List<Map<String, Object>> map) {
        LOGGER.info("path : {}", path);
        String style = path.substring(path.lastIndexOf("."), path.length()).toUpperCase(); // 从文件路径中获取文件的类型
        return generateWorkbook(path, style, map);
    }
    ...
    private static boolean generateWorkbook(String path, String style, List<Map<String, Object>> maps) {
        LOGGER.info("file style : {}", style);
        Workbook workbook;
        if ("XLS".equals(style.toUpperCase())) {
            workbook = new HSSFWorkbook();
        ...
        for (Map<String, Object> m : maps) {
            ...
            List<String> titles = (List<String>) m.get("titles");
            List<Map<String, Object>> values = (List<Map<String, Object>>) m.get("values");
            // 生成一个表格
            Sheet sheet;
            if (null == name || "".equals(name)) {
                sheet = workbook.createSheet(); // name 为空则使用默认值
